...
## 学んだこと
- **セレクタ**
<br>規則的なclass名、id名であれば、セレクタを使うことで簡略化できる。
<br>例えば`class="month1"`~`class="month12"`があったとして、CSSでそれら全てに同じスタイルを適用したい場面。
<br>`[class^="month"]`を使うことで「monthから始まる全てのclass」を指定できる。
<br>ほかにも`[class*=""]`, `[class$=""]`, `[class~=""]`などさまざまあるようなので便利。
